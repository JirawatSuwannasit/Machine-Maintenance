...
async function main() {
  ...
  const machines = dataRows.map((row, index) => {
    ...
    const machineId = record.Machine_ID.trim();
    const machineName = record.Machine_Name.trim();
    ...
    if (!VALID_STATUSES.has(status)) {
      skipped.push({ row: index + 2, machine_id: machineId, reason: `Invalid Status: ${status}` });
      return null;
    }
  ...
  const { data, error } = await supabase
    .from('machines')
    .upsert(machines, { onConflict: 'machine_id' })
    .select('machine_id');
