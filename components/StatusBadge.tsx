const styles: Record<string, string> = {
  ...
  DUE_SOON: 'bg-yellow-100 text-yellow-800 ring-yellow-200',
  OK: 'bg-emerald-100 text-emerald-800 ring-emerald-200',
};

export function StatusBadge({ value }: { value: string }) {
  return <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ring-1 ${styles[value] ?? 'bg-blue-100 text-blue-800 ring-blue-200'}`}>{value.replace('_', ' ')}</span>;
}
