...
If you do not keep a local `.env.local`, run the manual GitHub Actions workflow **Import machines** from the Actions tab. It reads `NEXT_PUBLIC_SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY` from GitHub repository secrets, installs dependencies, and runs `npm run import:machines`.
