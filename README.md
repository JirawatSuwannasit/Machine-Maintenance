...
A Supabase service-role key is not required for normal browser/server rendering. It is required only for server-side import jobs such as `npm run import:machines`; keep it server-only and never expose it to client components.
...
If you do not keep a local `.env.local`, run the manual GitHub Actions workflow **Import machines** from the Actions tab. The workflow is manual-only, reads `NEXT_PUBLIC_SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY` from GitHub repository secrets, installs dependencies, and runs `npm run import:machines`.
